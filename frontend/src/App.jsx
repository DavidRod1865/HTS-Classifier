...
const App = () => {
  ...
  const startNewClassification = async (searchQuery, isRefinement = false) => {
    ...
    try {
      ...
      const response = await fetch("/api/classify", {
        ...
        body: JSON.stringify({ 
          query: searchQuery,
          session_id: sessionId + "_" + Date.now(), // New session for each search
      ...
      if (data.success) {
        console.log(`✅ SUCCESS: ${actionType} classification started`);
  ...
  const continueConversation = async (responseText) => {
    console.log("🔄 CONTINUING CONVERSATION with response:", responseText);
    console.log("🔄 Current turn:", currentTurn);
    console.log("🔄 Session active:", conversationActive);
    
    setLoading(true);
    
    try {
      console.log("📡 Making API request to /api/classify (CONTINUE)");
      console.log("📡 Session ID:", sessionId);
      console.log("📡 This should continue existing conversation");
      ...
      if (data.success) {
        console.log("✅ SUCCESS: Conversation continued");
        console.log("📊 Debug info:", data.debug_info);
        console.log("🔍 Is continuation:", data.is_continuation);
        handleConversationalResponse(data);
      } else {
        console.log("❌ FAILURE: Conversation continuation failed:", data.error);
        setError(data.error || "Failed to continue conversation");
      }
    } catch (err) {
      console.error("💥 NETWORK ERROR:", err);
      setError(`Connection failed: ${err.message}`);
  ...
  const handleConversationalResponse = (data) => {
    ...
    // Update turn counter from backend response
    if (data.conversation_state && data.conversation_state.current_turn) {
      setCurrentTurn(data.conversation_state.current_turn);
      console.log("🔄 Turn updated to:", data.conversation_state.current_turn);
    }
    ...
    switch (data.type) {
      case 'clarifying_questions': {
        console.log("🔍 Processing clarifying_questions");
        const questionsToSet = data.data.questions || [];
        console.log("🔍 Questions to display:", questionsToSet);
        
        // Clear other states and set questions
        setResults([]);
        setOptions([]);
        setQuestions(questionsToSet);
        setAwaitingResponse(true); // We're now waiting for user response to questions
        break;
      }
        
      case 'multiple_options': {
        console.log("🔍 Processing multiple_options");
        const optionsToSet = data.data.options || [];
        console.log("🔍 Options to display:", optionsToSet);
        
        // Clear other states and set options
        setResults([]);
        setQuestions([]);
        setOptions(optionsToSet);
        setAwaitingResponse(true); // We're now waiting for user selection
        break;
      }
        
      case 'final_classification': {
        console.log("🔍 Processing final_classification");
        const finalResults = data.data.results || [data.data] || [];
        console.log("🔍 Final results:", finalResults);
        
        // Clear other states and set results
        setQuestions([]);
        setOptions([]);
        setResults(finalResults);
        setAwaitingResponse(false); // Conversation is complete
        break;
      }
        
      case 'high_confidence_results':
      case 'low_confidence_results': {
        console.log("🔍 Processing confidence results");
        const confidenceResults = data.data.results || [];
        console.log("🔍 Confidence results:", confidenceResults);
        
        // Clear other states and set results
        setQuestions([]);
        setOptions([]);
        setResults(confidenceResults);
        setAwaitingResponse(false); // Results received, no response needed
        break;
      }
      ...
      case 'no_results':
        ...
        setError("No HTS classifications found. Try being more specific.");
        break;
  ...
  const selectOption = async (optionIndex) => {
    ...
    // This is a response to Claude's multiple options - continue conversation
    setQuery(optionIndex.toString());
    await continueConversation(optionIndex.toString());
  };

  const respondToQuestion = async (response) => {
    if (!response.trim()) return;

    console.log("📝 Responding to Claude's question with:", response);
    
    // This is a response to questions - continue the conversation, don't start new search
    setAwaitingResponse(false); // We're no longer waiting for a response
    await continueConversation(response);
  };
  ...
  const startNewSearch = async () => {
    ...
    try {
      // Clear the conversation session on the backend
